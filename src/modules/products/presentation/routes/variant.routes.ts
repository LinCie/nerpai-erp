...
export const variantRoutes = new Elysia()
  ...
  .post("/products/:productId/attributes", async ({ params, body, organization }) => {
  ...
  .delete("/products/:productId/attributes/:attributeId", async ({ params, query, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
  ...
  .patch("/products/:productId/attributes/reorder", async ({ params, body, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
  ...
  // Generate variants
  .post("/products/:productId/variants/generate", async ({ params, body, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
  ...
  // Update variant
  .patch("/variants/:id", async ({ params, body, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
  ...
  .patch("/variants/:id/toggle", async ({ params, body, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
  ...
  // Soft delete variant
  .delete("/variants/:id", async ({ params, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
  ...
  .get("/products/:productId/variant-combinations", async ({ params, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
    ...
    return {
      ...
      existingCombinations: Array.from(result.existingCombinations).map(k => k.split("|")),
  ...
  .post("/variants/check-sku", async ({ body, organization }) => {
    if (!organization) throw new Error("Organization context is missing");
